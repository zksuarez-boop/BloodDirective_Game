...
{
    public static class MovementPrototype07Builder
    {
        private const string ScenePath = "Assets/_BloodDirective/Scenes/Prototypes/MovementPrototype07.unity";
        ...
        [MenuItem("Blood Directive/Reboot/Build Movement Prototype 07")]
        public static void BuildScene()
        {
            ...
            scene.name = "MovementPrototype07";

            Material floorMaterial = CreateMaterial("Prototype07_DarkConcreteFloor", new Color(0.04f, 0.055f, 0.065f));
            Material wallMaterial = CreateMaterial("Prototype07_BunkerConcrete", new Color(0.24f, 0.26f, 0.23f));
            Material metalMaterial = CreateMaterial("Prototype07_DarkMetal", new Color(0.055f, 0.065f, 0.07f));
            Material playerMaterial = CreateMaterial("Prototype_PlayerOlive", new Color(0.24f, 0.34f, 0.22f));
            Material enemyIdleMaterial = CreateMaterial("Prototype07_EnemyIdle", new Color(0.48f, 0.12f, 0.12f));
            Material enemyAlertMaterial = CreateMaterial("Prototype07_EnemyAlert", new Color(0.95f, 0.46f, 0.08f));
            Material enemyDamagedMaterial = CreateMaterial("Prototype07_EnemyDamaged", new Color(0.9f, 0.22f, 0.06f));
            Material enemyDefeatedMaterial = CreateMaterial("Prototype07_EnemyDefeated", new Color(0.08f, 0.08f, 0.08f));
            Material lightMaterial = CreateMaterial("Prototype07_ColdTubeLight", new Color(0.86f, 0.95f, 1f));
            Material rangeMaterial = CreateMaterial("Prototype07_AwarenessRangeMarker", new Color(0.16f, 0.32f, 0.34f));
            Material facingMaterial = CreateMaterial("Prototype07_EnemyFacingMarker", new Color(1f, 0.12f, 0.05f));
            Material playerDamagedMaterial = CreateMaterial("Prototype07_PlayerDamaged", new Color(0.58f, 0.34f, 0.18f));
            Material playerDefeatedMaterial = CreateMaterial("Prototype07_PlayerDefeated", new Color(0.08f, 0.08f, 0.08f));
            Material healthBackMaterial = CreateMaterial("Prototype07_HealthBack", new Color(0.08f, 0.02f, 0.02f));
            Material healthFillMaterial = CreateMaterial("Prototype07_HealthFill", new Color(0.9f, 0.05f, 0.04f));

            GameObject root = new GameObject("MovementPrototype07_Root");
            ...
            GameObject player = CreatePlayer(root.transform, playerMaterial, playerDamagedMaterial, playerDefeatedMaterial, healthBackMaterial, healthFillMaterial);
        ...
        private static GameObject CreatePlayer(Transform root, Material playerMaterial, Material damagedMaterial, Material defeatedMaterial, Material healthBackMaterial, Material healthFillMaterial)
        {
            ...
            var health = player.AddComponent<PrototypePlayerHealth>();
            SerializedObject healthSo = new SerializedObject(health);
            healthSo.FindProperty("_maxHealth").intValue = 5;
            healthSo.FindProperty("_renderer").objectReferenceValue = player.GetComponent<Renderer>();
            healthSo.FindProperty("_healthyMaterial").objectReferenceValue = playerMaterial;
            ...
            CreateHealthBar(player.transform, health, healthBackMaterial, healthFillMaterial);
            return player;
        }


        private static void CreateHealthBar(Transform player, PrototypePlayerHealth health, Material backMaterial, Material fillMaterial)
        {
            GameObject barRoot = new GameObject("PlayerHealthBar_VisualOnly");
            barRoot.layer = LayerMask.NameToLayer(VisualOnlyLayer);
            barRoot.transform.SetParent(player, false);
            barRoot.transform.localPosition = new Vector3(0f, 1.35f, 0f);

            GameObject back = GameObject.CreatePrimitive(PrimitiveType.Cube);
            back.name = "PlayerHealthBack_VisualOnly";
            back.layer = LayerMask.NameToLayer(VisualOnlyLayer);
            back.transform.SetParent(barRoot.transform, false);
            back.transform.localPosition = Vector3.zero;
            back.transform.localScale = new Vector3(1.75f, 0.08f, 0.12f);
            back.GetComponent<Renderer>().sharedMaterial = backMaterial;
            Object.DestroyImmediate(back.GetComponent<Collider>());

            GameObject fill = GameObject.CreatePrimitive(PrimitiveType.Cube);
            fill.name = "PlayerHealthFill_VisualOnly";
        ...
        private static GameObject CreateEnemy(Transform root, Material idleMaterial, Material alertMaterial, Material damagedMaterial, Material defeatedMaterial, Material facingMaterial, Transform target)
        {
            ...
            var chase = enemy.AddComponent<PrototypeEnemyChase>();
            SerializedObject chaseSo = new SerializedObject(chase);
            chaseSo.FindProperty("_target").objectReferenceValue = target;
            ...
            var contactDamage = enemy.AddComponent<PrototypeEnemyContactDamage>();
            SerializedObject contactSo = new SerializedObject(contactDamage);
            contactSo.FindProperty("_targetHealth").objectReferenceValue = target.GetComponent<PrototypePlayerHealth>();
        ...
        private static void EnsureLayer(int index, string layerName)
        {
            ...
            if (string.IsNullOrWhiteSpace(layer.stringValue))
                ...
                Debug.LogWarning($"[MovementPrototype07Builder] Layer {index} is already '{layer.stringValue}', expected '{layerName}'.");
