...
{
    public static class MovementPrototype05Builder
    {
        private const string ScenePath = "Assets/_BloodDirective/Scenes/Prototypes/MovementPrototype05.unity";
        ...
        [MenuItem("Blood Directive/Reboot/Build Movement Prototype 05")]
        public static void BuildScene()
        {
            ...
            scene.name = "MovementPrototype05";

            Material floorMaterial = CreateMaterial("Prototype05_DarkConcreteFloor", new Color(0.04f, 0.055f, 0.065f));
            Material wallMaterial = CreateMaterial("Prototype05_BunkerConcrete", new Color(0.24f, 0.26f, 0.23f));
            Material metalMaterial = CreateMaterial("Prototype05_DarkMetal", new Color(0.055f, 0.065f, 0.07f));
            Material playerMaterial = CreateMaterial("Prototype_PlayerOlive", new Color(0.24f, 0.34f, 0.22f));
            Material enemyMaterial = CreateMaterial("Prototype05_EnemyHealthy", new Color(0.55f, 0.16f, 0.14f));
            Material enemyDamagedMaterial = CreateMaterial("Prototype05_EnemyDamaged", new Color(0.85f, 0.42f, 0.08f));
            Material enemyDefeatedMaterial = CreateMaterial("Prototype05_EnemyDefeated", new Color(0.08f, 0.08f, 0.08f));
            Material lightMaterial = CreateMaterial("Prototype05_ColdTubeLight", new Color(0.86f, 0.95f, 1f));

            GameObject root = new GameObject("MovementPrototype05_Root");
            BuildWalkableLayout(root.transform, floorMaterial, metalMaterial);
            BuildBlockers(root.transform, wallMaterial, metalMaterial);
            BuildVisualOnlyDetails(root.transform, metalMaterial, lightMaterial);

            GameObject player = CreatePlayer(root.transform, playerMaterial);
            GameObject enemy = CreateEnemy(root.transform, enemyMaterial, enemyDamagedMaterial, enemyDefeatedMaterial);
        ...
        private static void BuildVisualOnlyDetails(Transform root, Material metalMaterial, Material lightMaterial)
        {
            ...
            CreateCube("EnemyRangeMarker_VisualOnly", new Vector3(0f, 0.16f, 3.2f), new Vector3(2.6f, 0.045f, 2.6f), metalMaterial, VisualOnlyLayer, root, false);
        }
        ...
        private static GameObject CreatePlayer(Transform root, Material playerMaterial)
        {
            ...
            player.transform.position = new Vector3(0f, 1f, -2.8f);
            player.GetComponent<Renderer>().sharedMaterial = playerMaterial;
            player.AddComponent<PrototypeClickMover>();
            return player;
        }

        private static GameObject CreateEnemy(Transform root, Material healthyMaterial, Material damagedMaterial, Material defeatedMaterial)
        {
            GameObject enemy = GameObject.CreatePrimitive(PrimitiveType.Capsule);
            enemy.name = "StationaryEnemy_Target";
            ...
            enemy.GetComponent<Renderer>().sharedMaterial = healthyMaterial;
        ...
        private static void EnsureLayer(int index, string layerName)
        {
            ...
            if (string.IsNullOrWhiteSpace(layer.stringValue))
                ...
                Debug.LogWarning($"[MovementPrototype05Builder] Layer {index} is already '{layer.stringValue}', expected '{layerName}'.");
